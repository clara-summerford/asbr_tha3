...
function [b_tip,b_post] = opt_pivot_calib(file,cal_file)
    ...
    fclose(fid);
    fid2 = fopen(optpivot_file_name, 'r');
    ...
    for i = 1:Nframes
        ...
        [R_D, p_D] = PC_registration(d_coords,D_coords{i});
        F_D{i} = [R_D, p_D'; zeros(1,3), 1];
    end
    
    % All F_D's are the same, so keep the first one
    F_D = F_D{1};
    ...
    for i = 1:Nframes
        for k = 1:Nh
            ...
            Hd = inv(F_D)*H'; % Hd = point H in EM tracker coordinates
            Hd(end) = [];
    ...
    H0 = sum(Hd_coords{1})/Nh; % Centroid of the observed points in frame 1
    ...
    for i = 1:Nframes
        ...
        [R,p] = PC_registration(h_coords,Hd_coords{i});
