...
% "calbody").
%
% Outputs: Cell array containing arrays of expected points where each cell
...
function C_est = C_expected(file1, file2)
    ...
    for i = 1:Nframes
        ...
        d_coords{i} = body_file(start:start+Nd-1,:);
        a_coords{i} = body_file(start+Nd:start+Nd+Na-1,:);
        c_coords{i} = body_file(start+Nd+Na:start+Nd+Na+Nc-1, :);
    end
    
    cal_d_coords = cal_file(1:cal_Nd,:);
    cal_a_coords = cal_file(cal_Nd+1:cal_Nd+cal_Na,:);
    ...
    for i = 1:Nframes
        ...
        for j = 1:length(cal_c_coords)
            ...
            % homogenour vector representation (last element should = 1)
